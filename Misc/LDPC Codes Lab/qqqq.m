...
for Pe = p
    ...
    for L = 1: Nsim
        ...
        for cn = 1: u
             for ind = 1: DC
                 if VN(CN(cn, ind), 2 * DV + 1) == 0
                     CN(cn, ind + DC) = 0;
                 elseif VN(CN(cn, ind), 2 * DV + 1) == 1
                     CN(cn, ind + DC) = 1;
                 elseif VN(CN(cn, ind), 2 * DV + 1) == 9
                     CN(cn, ind + DC) = 0.5;
                 end
             end
        end 

        % Load Message In This
        ...
        for t = 1: tmax
            ...
            for cn = 1:u
               for vn = 1:DC
                   ...
                   % this value
                   for vni = 1: DV
                       if VN(CN(cn,vn), vni) == cn
                           VN(CN(cn, vn), vni + DV) = 1 - prob0;
                           break;
                       end
                   end
               end
            ...
            for vn = 1:n
                for cn = 1:DV
                    ...
                    for cnvalue = DV + 1: 2 * DV
                        if cn + DV ~= cnvalue
                            lamda1 = lamda1 * (VN(vn, cnvalue)) / (1 - VN(vn, cnvalue));
                        end
                    end
                    for cni = 1: DC
            ...
            % Count Is decodedCur having erasure
            erasu = find(decodedCur == 9);
