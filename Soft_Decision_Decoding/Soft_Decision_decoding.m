    ...
    for L = 1: Nsim
        ...
        for t = 1: tmax
            ...
            for vn = 1: n
                ...
                for cn = DV + 1 : 2 * DV 
                    lamda1 = lamda1 * (VN(vn, cn)) / (1 - VN(vn, cn));
                end
                if lamda1 > 1
                    decodedCur(1, vn) = 1;
                elseif lamda1 == 1
                ...
                for cn = 1:DV
                    ...
                    for cni = 1: DC
                        if CN(VN(vn,cn), cni) == vn
                            CN(VN(vn, cn), cni + DC) = lamda1 / (1 + lamda1);
                        end
                    end
                end
            end 
...
pTheor = zeros(length(p), tmax); % i am the brother of the algorithmic convergence but better!
for i = 1: length(p) % as done in algorithmic convergence take one probability and match corresponding all erasure_avg
    pTheor(i, 1) = p(i);% take probability(take a note if you want number of erasure than you can say n * p(i)
    % above is just modified version of the algorithmic convergence
    ...
    plot(pTheor(i, :), 'DisplayName',['p=' num2str(p(i))]); % i am better performer than algorithmic
end
toc
